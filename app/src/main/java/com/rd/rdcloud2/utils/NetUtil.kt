...
import com.rd.rdcloud2.utils.NetType
...
object NetUtils {
    ...
    fun getSsid(wifiInfo: WifiInfo?): String? {
        ...
        if (wifiInfo != null) {
            ...
            ssid = if (wifiInfo.ssid.startsWith("\"") && wifiInfo.ssid.endsWith("\"")) {
     ...
     * 获取wifi IP地址
     * @param context Context
     * @return String 返回IP地址 192.168.1.123
     */
    fun getWifiIP(wifiInfo:WifiInfo): String {
        val ipInt = wifiInfo.ipAddress
        return intToIp(ipInt)
    }

    private fun intToIp(ipInt: Int): String {
        return ((ipInt and 0xFF).toString() + "." + (ipInt shr 8 and 0xFF) + "." + (ipInt shr 16 and 0xFF)
